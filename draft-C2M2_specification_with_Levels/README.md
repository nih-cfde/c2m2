...
| `creation_time` | An ISO 8601 / RFC 3339 (subset)-compliant timestamp documenting this entity's creation time (or, in the case of a `subject` entity, the time at which the `subject` was first documented by the `project` under which the `subject` was first observed): **YYYY-MM-DDTHH:MM:SS±NN:NN**, where **YYYY** is a four-digit Gregorian **year**; **MM** is a zero-padded, one-based, two-digit **month** between `01` and `12`, inclusive; **DD** is a zero-padded, one-based, two-digit **day** of the month (between `01` and `31`, inclusive); **HH** is a zero-padded, zero-based, two-digit **hour** label between `00` and `23`, inclusive (12-hour time is specifically prohibited); **MM** and **SS** represent zero-padded, zero-based Babylonian sexagesimal integers (between `00` and `59`, inclusive) denoting **minutes** and **seconds**, respectively; **±** denotes exactly one of `+` or `-`, indicating offset (in hours and minutes) from GMT, qualifying the following time zone encoding (or `-` in the case in which no time zone data is asserted); and **NN:NN** represents the **hours:minutes** differential between the local time zone of this `creation_time` and GMT, qualified as appropriate by a preceding `+` or `-` indicating offset direction, with `-00:00` encoding the special case in which no time zone data is asserted. Except for the time zone portion of `creation_time` (just described) and the **year YYYY**, all other component portions of `creation_time` may be zeroed (`00`) to indicate a lack of corresponding precision. We are aware (and unconcerned) that this technically renders the value `00:00:00` ambiguous. |
