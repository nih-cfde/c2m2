...
| `id` | Unrestricted-format **string identifying this `file`, assigned by the DCC managing it**. Can be any string as long as it **uniquely identifies each `file`** within the scope of a single Level 0 `id_namespace` value. |
...
##### Core entities
   ...
   * `subject` _introduced (also cf. below, §"Common fields" and §"Taxonomy and the `subject` entity")_
      * _The Level 1_ `subject` _entity is a generic container meant to represent any biological
      ...
      Level 2: cf._ `biosample` _, immediately above)_
